...
public class SeqLogReg {
    ...
    /** Learned model weight vector, used for future predictions */
        ...
        public def run() {
                ...
                logistic.map(y, o, (y_i:ElemType, o_i:ElemType)=> (1.0f / (1.0f + Math.exp(-y_i * o_i))) as ElemType);
                //obj = 0.5 * t(w) %*% w + C*sum(logistic)
                val obj = 0.5f * w.dot(w) + C*logistic.sum();
		...
		val logisticD = new Vector(logistic.M, (i:Long)=> (logistic(i)*(1.0f-logistic(i))) as ElemType);
...
//                      logisticnew = 1.0/(1.0 + exp(-y * o ))
			logisticnew.map(y, o, (y_i:ElemType, o_i:ElemType)=> (1.0f / (1.0f + Math.exp(-y_i * o_i))) as ElemType);
//                      objnew = 0.5 * t(wnew) %*% wnew + C * sum(logisticnew)
                        val objnew = 0.5f * wnew.dot(wnew) + C * logisticnew.sum();
...
//                              grad = w + C*t(X) %*% ((logisticnew - 1) * y )
        ...
        protected def compute_grad(grad:Vector(X.N), logistic:Vector(X.M)):void {
                //grad = w + C*t(X) %*% ((logistic - 1)*y)
        logistic.map(y, (x:ElemType, v:ElemType)=> {(x - 1.0f) * v});
