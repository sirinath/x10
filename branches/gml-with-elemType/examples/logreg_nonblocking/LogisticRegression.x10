...
public class LogisticRegression {
    ...
    val eta0 = 0.0 as ElemType;
    val eta1 = 0.25 as ElemType;
    val eta2 = 0.75 as ElemType;
    val sigma1 = 0.25 as ElemType;
    val sigma2 = 0.5 as ElemType;
    val sigma3 = 4.0 as ElemType;
    val psi = 0.1 as ElemType;  
	...
	val obj = 0.5 * w.norm(w) + C*logistic.sum() as ElemType; 
	...
	while(!converge) {
	    ...
	    //                      qk = -0.5*(t(s) %*% (grad - r))
	    val qk = -0.5 * s.norm(grad-r) as ElemType;
	    //                      
	    //                      wnew = w + s
	    w.copyTo(wnew);
	    wnew.cellAdd(s);
	    ...
	    logisticnew.scale(-1 as ElemType).cellMult(o).cellAdd(1.0 as ElemType).cellDivBy(1.0 as ElemType);
	    
	    //                      objnew = 0.5 * t(wnew) %*% wnew + C * sum(logisticnew)
	    val objnew = 0.5 * wnew.norm(wnew) + C * logisticnew.sum() as ElemType;
	    //                      
	    //                      rho = (objnew - obj) / qk
