...
class TestRunSumma {
	...
	var ret:Boolean = true;
	val a = DistBlockMatrix.makeDense(gA, dA).init((r:Long,c:Long)=>ET(1.0*(r+c+1)));
	val b = DistBlockMatrix.makeDense(gB, dB).init((r:Long,c:Long)=>ET(2.0*(r*c+1)));
	...
	Console.OUT.printf("distributed in (%dx%d) places\n", gdA.numRowPlaces, gdA.numColPlaces);
	var ret:Boolean = true;
	val a = DistBlockMatrix.makeSparse(gA, dA, nzd).init((r:Long,c:Long)=>ET(1.0*(r+c)));
	val b = DistBlockMatrix.makeSparse(gB, dB, nzd).init((r:Long,c:Long)=>ET(1.0*(r+c)));
	val c = DistBlockMatrix.makeDense(gC, dC);
	SummaMult.mult(a, b, c, false);
